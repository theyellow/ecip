...
| 3.5a | ① CI net | **Require status checks on `main`** — the `Branchruleset` ruleset enforces PR-only merges but has no `required_status_checks` rule, so a red (or entirely absent) build still merges; #229 merged during the 2026-08-06 Actions outage with zero checks. Add the `Java CI with Maven` jobs as required. Makes 3.1–3.3 actually *gate* rather than merely report. | CI-GATE | XS | — | 3.1, 3.2 |
