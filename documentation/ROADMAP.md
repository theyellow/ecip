...
|-------|------|------|----|------|-------|-------|
| 3.1 | ① CI net | ✅ **Java integration tests in CI** — failsafe was configured in root `pluginManagement` but never listed in `<build><plugins>`, and it isn't a default-lifecycle plugin, so it never ran; surefire excluded `*IT` **and** `*IntegrationTest`, hiding 15 classes / 27 methods. Failsafe activated at root (inherited by all modules), CI switched to `mvn -B verify`, hardcoded 4-IT list dropped. The first CI run immediately caught a latent order-dependent bug no local run reproduced: `AbstractModerationIntegrationTest` restarted its Testcontainers per class while the cached Spring context kept the old ports → health 503. Fixed via the singleton pattern. (PR #227) | INF-CI-IT | M | — | — |
| 3.2 | ① CI net | ✅ **Frontend tests in CI gate** — `npm-test` execution (`vitest run`) bound to the `test` phase, skipped under `-DskipTests`. 27 files / 145 tests, all green, ~24s. (PR #227) | INF-CI-FE | XS | — | — |
| 3.3 | ① CI net | ✅ **`@PreAuthorize` live-filter e2e test** — `PreAuthorizeEnforcementTest` drives the real `SecurityConfig` chain with real JWTs (not `@WithMockUser`, which would skip `JwtAuthenticationFilter` + `RolePermissions`). Caught a live bug on first run: every authorization denial returned **500 instead of 403**, because `GlobalExceptionHandler`'s catch-all claimed `AccessDeniedException` before Spring Security could translate it (AUTHZ-500, fixed). Follow-up AUDIT-DENY (method-level denials went unaudited, leaving P2.8's trail missing the majority of denials) fixed straight after. (PR #228, #229) | P1-M1 | S | — | 3.1 |
| 3.4 | ① CI net | ✅ **Coverage gate now exists in all 11 modules, ratcheted at measured−2** — JaCoCo was configured in root `<pluginManagement>` but only `emcip-core` declared it in `<build><plugins>`, and it is not a default-lifecycle plugin, so ten of eleven Java modules had no coverage measured at all; the gate did not exist. It now runs in all 11 modules on merged unit+IT execution data, with a per-module ratchet floor at `measured − 2` (root default `0.00`, so a new module inherits a floor it cannot fail until its own PR sets one). 80% is the ratchet's destination, not a wall hit at once — tracked as per-module follow-ups in BACKLOG §0b. Floors: admin-api 0.53, admin-ui 0.74, audit-service 0.71, conversation-context 0.72, core 0.78, intent-classifier 0.69, knowledge-engine 0.73, llm-orchestrator 0.52, moderation-service 0.83, policy-engine 0.70, tdlib-adapter 0.42. See the note below the table for corrections and the verification lesson from this work. | #4 | L | — | 3.1, 3.2 |
| 3.5 | ① CI net | ⏸️ **Deferred (scheduling decision, not a rejection)** — **Gatling load tests in CI**, 3 sims exist, add gate. Deferred out of P3 on 2026-08-09: there is no reliable build/test environment to run load against (the dev cluster is hand-wired and single-replica), the account is free-tier so CI minutes are the binding constraint, and current performance is adequate with no reported latency problem to gate against. A load gate run on an unrepresentative target measures the target, not the code — the exact "check that cannot fail" shape P3.4 caught three times. The three simulations remain in the repo and are runnable by hand. Revisit when performance becomes a concern, when a representative environment exists, or when CI capacity stops being the constraint. | #14 | S | — | — |
...
| 3.6 | ② Security | ✅ **Per-user/IP rate limiting** — the three Resilience4j limiters were **global counters**: one caller exhausting `admin-crud` locked out every operator, which is a self-inflicted DoS rather than a defence. Replaced by `RateLimitWebFilter`, which keys `auth` by trusted client IP and everything else by JWT subject, holding limiters per key in a bounded Caffeine cache (10 000/group, 2-min idle TTL). Three findings on the way: (a) keying by IP is worthless while the IP comes from the leftmost `X-Forwarded-For` token — an attacker mints a fresh bucket per request — which made **P2.8-F1 a hard prerequisite**, not a follow-up, so `ClientIp` now counts hops from the *right* and the BFF appends its own hop instead of passing the client's copy through; (b) `POST /api/flags/{id}/reply` had **no limiter at all** under the per-controller scheme, so coverage is now structural — an unmatched `/api/**` path falls through to `admin-crud` fail-closed rather than escaping limits; (c) throwing `RequestNotPermitted` from a `WebFilter` would have returned **500, not 429**, because `GlobalExceptionHandler` is a `@RestControllerAdvice` and never sees filter-thrown exceptions — the identical trap to AUTHZ-500 in 3.3 — so the filter writes its own response, byte-identical to the handler's. ✅ **Verified live 2026-08-09 — and the assumed value was wrong.** `trusted-proxy-hops` was `2`, assuming a BFF hop; the ingress actually routes `/api/**` straight to admin-api, so the real count is **1**. The failure was silent by construction: every request fell back to the socket address, collapsing all callers into one bucket, so per-IP limiting did nothing while looking healthy — only `emcip.ratelimit.untrusted_ip` showed it. Corrected to 1 (declared in `helm/emcip/values.yaml`, since it describes topology, not preference) and re-verified: 12 requests with 12 different spoofed `X-Forwarded-For` values share one bucket and the fallback counter never registers. Live 429 confirmed with `Retry-After: 60` and the ProblemDetail body — not a 500. This is the entire argument for the verify-live step: the value no test can check was the one that was wrong. | RT-F1 | S | — | — |
| 3.7 | ② Security | ✅ **Secrets: startup self-check** — **premise correction:** reads were never lenient-fail-closed; `SecretCipher.decrypt()` has thrown on any unprefixed value since P2.0 and both admin-api and llm-orchestrator already translated that into a 409. The real gap was *when* plaintext is discovered — lazily, on first operator touch, not at boot — and that is what this delivers: an `ApplicationRunner`-based scan in `emcip-core`, wired into admin-api, knowledge-engine and llm-orchestrator, reporting `OK`/`PLAINTEXT`/`KEY_MISMATCH`/`UNVERIFIED` per column via log, `emcip.secrets.*` gauges, and an always-`UP` health indicator, plus an hourly re-scan. Ships `warn` (log + boot) everywhere; `fail` (refuse to boot) is opt-in per environment only after a clean re-scan cycle, because a hard fail would deadlock the #241/#243 in-product repair paths. Found along the way: llm-orchestrator was missing `@EnableScheduling` (the re-scan would never have fired); the metric read `0` — indistinguishable from "clean" — for an unscanned column, fixed to `NaN`; an uncaught `SQLException` at boot would have failed startup even under `warn`, fixed so `warn` logs and continues. No environment promoted to `fail`; live cluster state not yet read. Details: `docs/superpowers/specs/2026-08-15-secrets-startup-self-check-design.md`, `docs/superpowers/BACKLOG.md` (P2.0-F1), `docs/operations/secrets-encryption.md`.  **⚠️ The first cluster deploy crash-looped admin-api for 7 days** (no `DataSource` bean for the self-check in this R2DBC service; fixed in #246, guarded by `AdminApiBootIT` — see `docs/operations/incidents/2026-09-13-admin-api-crashloop.md`). **✅ Verified live 2026-09-24 (SELFCHECK-F3):** all four columns scanned in all three services, no plaintext and no key mismatch, the two empty columns UNVERIFIED as expected, and the hourly re-scan proven to fire. The verify-live step again found what no test could: knowledge-engine had never been a Prometheus scrape target. This matters more than it looks: the entire feature is a claim about *live data*, which no test can observe — the exact P3.6 shape where the one value no test could check (`trusted-proxy-hops`) was the one that was wrong. Promotion to `fail` is now permitted for the dev cluster but still a deliberate decision (needs SELFCHECK-F5's Helm plumbing). 3.7 is **not** a 1.0.0 gate row. | P2.0-F1 | S | — | — |
...
| 3.8a | ② Security | ✅ **Knowledge search tenant scoping** — **premise correction:** not one method but three search queries treated a null tenant as *every tenant*, and admin-api's knowledge proxy let tenant-bound roles choose any tenant (reachable cross-tenant read). Rule: tenant → own + global; null → global only; the proxy binds the tenant from the caller's identity. Tenant-scoped graph search, empty before (all live knowledge is global), now returns the global graph. Follow-ups KNOW-F1 (research/ingestion proxies) and KNOW-F2 (`findConnected`). The 3.14 multi-tenancy ADR must record the rule. Details: BACKLOG TENANT-F1, `docs/superpowers/specs/2026-09-25-p3.8a-knowledge-search-tenant-scoping-design.md`. | TENANT-F1 | M | ✅ **gate** | — |
...
| 3.14 | ④ Decisions | **ADRs 009–011** — ✅ **ADR-009 multi-tenancy** (2026-09-25, with KNOW-F1: tenant sources, own + global visibility, search vs admin-view meaning of "no tenant", global writes only by ADMIN, system templates global, enforcement layers); auth/authz architecture (ADR-010 — must also decide **internal service authentication**: knowledge-engine and Kafka/RT-005 trust any in-cluster caller today); **API versioning (decide `/v1` before 1.0.0)**. Settled *before* Tier ⑤ adds the new tenant endpoint. | S6 | S | ✅ **gate** | — |
...
| 3.20 | ⑤ Infra | **K8s pod hardening** — `securityContext` (readOnlyRootFilesystem, runAsNonRoot, drop caps), ServiceAccounts, NetworkPolicy — incl. **knowledge-engine ingress limited to admin-api and llm-orchestrator** (ADR-009 rule 5 treats a caller without a tenant as trusted; no NetworkPolicy exists today). | RT-012 | M | ✅ **gate** | — |
...
| 3.23 | ⑤ Infra | **Distributed tracing is not wired — no service emits traces** — all **9** Spring services declare `micrometer-tracing-bridge-otel` + the OTel SDK and set `management.tracing.*` / `management.otlp.*`, but **none** pull Spring Boot 4's tracing autoconfiguration, which Boot 4 split out of `spring-boot-starter-actuator` into `spring-boot-starter-opentelemetry`. No `Tracer` bean is created, so the tracing config is dead and nothing is exported. A Boot 3→4 migration leftover of the same class as the Jackson 2→3 issue: configuration that looks correct and silently does nothing. Fix is a per-service dependency swap, but must be verified properly (assert a `Tracer` bean exists **and** that spans reach an OTLP collector) — the existing `TraceContextPropagationIT` passed throughout because it only asserts property values and "registry != NOOP", which the metrics handler alone satisfies. Found 2026-08-07 during P3.4. | TRACE-0 | S | ✅ **gate** | — |

> **Corrections to row 3.4's original claims.** The row previously named intent-classifier as one of the three weakest modules — it measures 71.5%, actually the **second-best** of the eleven. The genuinely worst module was **conversation-context** at 22.1% (now 74.8% after targeted tests) — it wasn't named at all. **knowledge-engine** held the single largest block of uncovered code, 983 missed lines, and also wasn't mentioned. **moderation-service**, which the row called weakest-against-target, in fact measures 85.4% — already above the 80% goal.
>
> **Verification lesson (the transferable finding from this work).** P3.4 shipped three separate checks that could not fail, each caught only when something forced it to prove it could:
> 1. `<argLine>${surefireArgLine}</argLine>` — Maven interpolates `${...}` at model-build time, so the empty property was baked into plugin config before `prepare-agent` ran and set it. Result: `BUILD SUCCESS` across 13 modules with 193 test classes and **zero** `.exec` files, because `jacoco:check` skips rather than fails when data is missing.
> 2. The exclude assertion `grep -hE ',(dto|entity)$'` against `jacoco.csv` — every row ends with a numeric column, so the pattern matched nothing whether the excludes worked or had been deleted.
> 3. `TraceContextPropagationIT` and its three sibling tests passed throughout while distributed tracing was dead platform-wide (see 3.23) — they asserted property values and "registry != NOOP", never that a `Tracer` bean existed.
>
> The rule this implies: **a check that has never been observed failing is not evidence.** 3.23's tracing fix must therefore prove a `Tracer` bean exists and that spans reach a collector, not merely that configuration values look correct.

**1.0.0 release gate:** P1 + P2 ✅, plus every **Gate**-flagged row above — the tenant-scoping decision for `findSimilarNodes` (3.8a), the boot-from-zero infra (3.16–3.19), K8s pod hardening (3.20), Prometheus alerting (3.22), distributed tracing (3.23), and the ADRs incl. the `/v1` decision (3.14). Non-gate rows are strongly recommended before tagging but do not block it. Ordered value-first, so the gate items cluster in the final stretch. (Rows folded in from BACKLOG §0b + the old P4 polish bucket during the 2026-08-05 cleanup; BACKLOG §0b remains the status source of truth for the remediation follow-ups.)

> ### ✅ Cluster availability — constraint lifted 2026-09-24 (was noted 2026-08-18)
>
> From 2026-08-18 no cluster was available to verify against, which blocked **closing** the rows
> below. A cluster is available again as of 2026-09-24, so they can now be closed — but each still
> needs its live proof, not just merged code:
>
> | Row | Gate? | Why it needs a cluster |
> |-----|-------|------------------------|
> | 3.18 Telegram seeding via Helm | ✅ | A post-deploy Job is only proven by deploying it. |
> | 3.19 Fresh-install smoke test | ✅ | *Is* the boot-from-zero validation; explicitly "do with the user". |
> | 3.20 K8s pod hardening | ✅ | `readOnlyRootFilesystem` etc. break at runtime, not at build. |
> | 3.22 Prometheus alerting rules | ✅ | A rule that never fired is not evidence — the P3.4 verification lesson (see the note under this table) applies directly. |
> | 3.23 Distributed tracing | ✅ | The row itself demands proof that **spans reach a collector**, precisely because the existing tests passed while tracing was dead. |
> | 3.16 Liquibase consolidation | ✅ | Squashing can be written and unit-tested, but a consolidated changelog is only trustworthy once applied to a real empty database. |
> | ~~SELFCHECK-F3 (3.7 follow-up)~~ | — | ✅ Done 2026-09-24. |
>
> **Recommended sequencing:** SELFCHECK-F3 (2026-09-24) and 3.8a (2026-09-25) are done. Next
> **KNOW-F1** (the 3.8a hole in the research/ingestion proxies) and the gate row **3.14**, then the
> polish tier. (COST-LOG and PROMPT-TENANT, both found by the F3 deploy, were fixed
