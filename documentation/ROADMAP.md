...
| 3.3 | ① CI net | ✅ **`@PreAuthorize` live-filter e2e test** — `PreAuthorizeEnforcementTest` drives the real `SecurityConfig` chain with real JWTs (not `@WithMockUser`, which would skip `JwtAuthenticationFilter` + `RolePermissions`). Caught a live bug on first run: every authorization denial returned **500 instead of 403**, because `GlobalExceptionHandler`'s catch-all claimed `AccessDeniedException` before Spring Security could translate it (AUTHZ-500, fixed). Follow-up AUDIT-DENY logged: method-level denials are still unaudited. | P1-M1 | S | — | 3.1 |
