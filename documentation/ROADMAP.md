...
| 3.8a | ② Security | **`findSimilarNodes` performs a cross-tenant read when tenant is null** — `GraphNodeEmbeddingRepository.findSimilarNodes(embedding, null, limit)` emits `WHERE embedding IS NOT NULL` with **no tenant predicate**, returning graph nodes from *every* tenant. Its sibling `findNearestNeighbour(…, null, …)` emits `tenant_id IS NULL`, i.e. scoped to unowned rows — the two methods disagree on what a null tenant means. Reachable: `KnowledgeQueryService.java:84` passes `request.tenantId()` straight through, so the safety of the unfiltered branch rests entirely on an upstream non-null guarantee that is asserted nowhere. The javadoc ("optionally filtered by tenant") suggests the global branch may be deliberate — so **decide the semantics** (reject null / scope to `IS NULL` / keep global and document it) rather than patching one method. Note `GraphNodeEmbeddingRepositoryTest` currently characterises today's behaviour (`sql.doesNotContain("tenant_id")`) and must be updated with whatever is decided. Found 2026-08-08 during P3.4 while writing tenant-filter tests. | TENANT-F1 | S | ✅ **gate** | — |
...
> ### ⛔ Cluster availability is now a P3 sequencing constraint (noted 2026-08-18)
>
> No cluster is currently available to verify against. That does not block *writing* code, but it
> does block **closing** several rows — and most of them are 1.0.0 **gate** items, so this is a
> release-schedule issue, not a nuisance:
>
...
> | SELFCHECK-F3 (3.7 follow-up) | — | Reading the live gauges is the whole point of the step. |
>
> **What can still proceed with no cluster:** 3.8 (base-image pinning), **3.8a** (`findSimilarNodes`
> tenant scoping — a gate row, and pure code + tests), 3.9–3.13 (product polish), 3.14 ADRs
> (a gate row, docs only), 3.15 backup/restore runbook, 3.17 tenant provisioning endpoint
> (buildable and testable; only its *seed* path wants a cluster).
>
> **Recommended sequencing while the cluster is unavailable:** take the gate rows that need no
> cluster first — **3.8a** and **3.14** — then the polish tier, and hold the Tier ⑤ sprint intact
> until a cluster exists rather than half-landing it. Splitting 3.16–3.19 across a cluster gap is
> how drift like the current hand-wired cluster happened in the first place.
