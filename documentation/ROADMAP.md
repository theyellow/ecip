...
|-------|------|------|----|------|-------|-------|
| 3.1 | ① CI net | ✅ **Java integration tests in CI** — failsafe was configured in root `pluginManagement` but never listed in `<build><plugins>`, and it isn't a default-lifecycle plugin, so it never ran; surefire excluded `*IT` **and** `*IntegrationTest`, hiding 15 classes / 27 methods. Failsafe activated at root (inherited by all modules), CI switched to `mvn -B verify`, hardcoded 4-IT list dropped. Triage found **zero latent failures**. | INF-CI-IT | M | — | — |
