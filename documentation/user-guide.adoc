...
==== Marking a Flag as Reviewed

Once you have inspected a flagged message, click *Mark reviewed* in the row. The status badge changes from `NEW` to `REVIEWED`. This action is recorded via `PATCH /api/flags/\{id}/status` and is permanent (there is no undo in the UI, though the API accepts any status value).

NOTE: The *Mark reviewed* button is hidden for rows already in `REVIEWED` or `ACTIONED` status.
...
[source,bash]
----
curl "http://localhost:9087/api/flags?size=25&decision=FLAG" \
  -H "Authorization: Bearer <token>"
----
...
----
curl -X PATCH \
  "http://localhost:9087/api/flags/23dcbc18-4a19-48c6-a6bd-259dcbfd76f8/status" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"status": "REVIEWED"}'
----
