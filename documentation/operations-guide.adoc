...
|REST via admin-api
|Management operations: synchronous queries from operators or the Admin UI. `emcip-admin-api` holds typed `WebClient` proxy clients for each domain service and authenticates with `X-Service-Token`.
|===
