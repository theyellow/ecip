...
The local Docker Compose environment runs 9 application services and 9 infrastructure services:
...
moderation-service, audit-service, admin-api, knowledge-engine
...
NOTE: All services connect to the same PostgreSQL instance but each service owns its own tables.
No service accesses another service's tables. Cross-service data access uses Kafka (pipeline) or
REST via `emcip-admin-api` (management). Liquibase migrations in each service create and manage
only that service's tables.
...
|9088
|Knowledge management (graph + vector search)
...
|Prometheus
|14010
|Metrics storage (scraped by Grafana)
...
[source,bash]
----
for port in 9080 9081 9082 9083 9084 9085 9086 9087 9088 \
...
[source,bash]
----
for port in 9080 9081 9082 9083 9084 9085 9086 9087 9088; do
  echo -n "Port $port: "
