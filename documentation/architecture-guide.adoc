...
|9088
|Ontology-driven knowledge management; graph (Apache AGE) + vector (pgvector) search; consumes `knowledge.raw.messages`, publishes `knowledge.events`. JPA / PostgreSQL.
|===
...
|3
|knowledge-engine
|admin-api, llm-orchestrator
|`eventId`, `eventType` (EXTRACTION_COMPLETE, ENRICHMENT_REQUEST, ENRICHMENT_RESPONSE, BACKFILL_PROGRESS), `payload` (JSONB)
|===
