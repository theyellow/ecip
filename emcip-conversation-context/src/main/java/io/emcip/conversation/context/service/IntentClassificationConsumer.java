 ...
 * Consumes from messages.classified topic and updates message records.
 */
@Service
public class IntentClassificationConsumer {
    ...
    public void consume(ConsumerRecord<String, String> record) {
        ...
        Mono.fromCallable(() -> {
                    ...
                    var validation = eventValidator.validateJson(record.value(), "IntentClassified");
                    if (!validation.valid()) {
                        log.error("Invalid classification received: {}", validation.getErrorMessage());
                        return null;
                    }

                    // Parse and process
                    EventSchemas.IntentClassifiedEvent event =
                            objectMapper.readValue(record.value(), EventSchemas.IntentClassifiedEvent.class);
                ...
                .doOnSuccess(result -> {
                    if (result != null) {
                        log.info("Updated intent classification for source event {}: {}",
                                result.sourceEventId(), result.intent());
                    }
                })
