...
/**
 * Repository for Message entities. Provides CRUD operations and custom queries for message
 * retrieval.
 */
@Repository
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    Optional<Message> findByTelegramMessageIdAndThread_TelegramChatId(
            Long telegramMessageId, Long chatId);
    ...
    @Transactional
    @Query(
            "UPDATE Message m SET m.isDeleted = true WHERE m.telegramMessageId = :messageId AND"
                    + " m.thread.telegramChatId = :chatId")
    ...
    @Transactional
    @Query(
            "UPDATE Message m SET m.isEdited = true, m.editTimestamp = :editTime WHERE"
                    + " m.telegramMessageId = :messageId AND m.thread.telegramChatId = :chatId")
    int markAsEdited(
            @Param("messageId") Long messageId,
            @Param("chatId") Long chatId,
            @Param("editTime") Instant editTime);

    /** Update intent classification for a message. */
    @Modifying
    @Transactional
    @Query(
            "UPDATE Message m SET m.intentClassification = :intent, m.confidenceScore = :confidence"
                    + " WHERE m.eventId = :eventId")
    int updateIntentClassification(
    ...
    @Query(
            "SELECT m.intentClassification, COUNT(m) FROM Message m WHERE m.intentClassification IS"
                    + " NOT NULL GROUP BY m.intentClassification")
