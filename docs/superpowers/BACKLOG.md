...
> Last updated: 2026-06-10 (XS sweep #30/#31/#35 done; #32/#22 done; #33 done; #24 Phase 2 wired/untested; #34 moderation-service rewire done; #36 signal detectors done; #39 Decisions page filters + pagination done; SC6b promoted from Deferred; #40/#41/#42 added from POSSIBLE_DEVELOPMENT.md audit)
...
| 39 | **Decisions page: filters + pagination** | S | ✅ 2026-06-09. Renamed Flags→Decisions. Fixed DECISIONS constant (ALLOW/BLOCK/FLAG/RESPOND/ESCALATE/REVIEW/EXECUTE). Added intent/from/to/minConfidence filters through policy-engine → admin-api → UI. Time-period presets (Today, 7d, 30d, This month, Last month, Custom). Pagination controls. `color-scheme` dark/light fix for native datetime pickers. |
| SC6b | **Audit-log page: filters + pagination** | S | Audit-log page has the same hard-200-limit and no-filter problem #39 just fixed for Decisions. audit-service needs `page`/`size` + time-range filters on `GET /api/audit/events` (spec: `docs/superpowers/specs/2026-05-20-sc6-sc7-sc8-design.md §SC6`); thread through `AuditServiceClient` → `AuditController` (admin-api) → UI. See also the old SC6b in Deferred (PolicyRuleController cap — that stays deferred, it's config data). |
| 40 | **SC8 resilience follow-ons** | S | Two gaps left after circuit breakers landed (PR #73): (1) retry with exponential backoff on circuit-broken calls before surfacing 503; (2) per-service fallback responses (empty list / degraded payload) so the admin UI stays functional when one downstream is down. Spec: `docs/superpowers/specs/2026-05-20-sc6-sc7-sc8-design.md §SC8`. |
| 41 | **Admin UI: design-handoff deferred UX** | M | Three items explicitly deferred from the v2 handoff (PR #94): (1) **Simulate** two-column layout with real-time animated pipeline trace (stage-by-stage progress: `telegram.raw.messages` → `messages.classified` → `policies.decisions`); (2) **Decisions** reply composer v2 — 4-mode SegmentedControl (Public/Quote-reply/Private DM/Silent note), chip-row templates, `{n} chars · MODE` footer counter; (3) **Users** expanded role model (MODERATOR, ANALYST, VIEWER) with `lastLogin`/`createdAt` columns — needs backend role expansion. Items (2) and (3) need backend work before UI can follow. |
...
| SC6b | **Paginate `PolicyRuleController.listActive()`** | XS | SC6 adds a `.take(200)` safety cap. Full `PageResponse<T>` not needed — policy rules are config data and will stay small in practice. Revisit if a tenant exceeds ~100 rules. Note: audit-service pagination (also originally SC6) has been promoted to active as backlog item SC6b above. |
