...
| CI-DUP | **`code-quality` duplicates `build`.** Found during P3.5a (CI-GATE) while reading `maven.yml` to design the `ci-gate` aggregator: `build` already runs `spotless:check` and `pmd:check` (`.github/workflows/maven.yml:62-66`), and `code-quality` runs a second `mvn clean install -DskipTests` solely to re-run the same two checks — roughly a full build's worth of CI time wasted per PR. Deliberately left alone in P3.5a to keep that change reviewable on its own; fold `code-quality`'s checks into `build` (or vice versa) and drop the duplicate job. Ref: `.github/workflows/maven.yml`. | LOW | P3/P4 | XS | ⏳ |
