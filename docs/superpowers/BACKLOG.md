...
| 35 | **Self-host Inter Variable font (offline capability)** | XS | Inter is currently loaded from Google Fonts CDN (`index.html`). When the cluster moves to a fully isolated network (microk8s DMZ, no external internet from pods), the CDN link will fail silently and `system-ui` will take over — causing minor layout shifts. Fix: download `Inter-Variable.ttf` from rsms.me/inter or the `inter-font` npm package, place in `emcip-admin-ui/src/main/frontend/public/fonts/`, and add a `@font-face` block to `variables.css` matching the existing Cinzel/SCP pattern (`font-weight: 100 900; font-display: swap`). Remove the Google Fonts `<link>` from `index.html`. Prerequisite: offline-capable cluster design (no items block this). |
