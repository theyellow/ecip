...
| CI-GATE | **`main` ruleset does not require status checks.** The `Branchruleset` ruleset (id 15118295, `enforcement=active`, targets `refs/heads/main`, no bypass actors) enforces `deletion`, `non_fast_forward` and `pull_request` (`required_approving_review_count=0`) — but has **no `required_status_checks` rule**. So a PR can merge with a red build, or with no checks at all: PR #229 merged during the 2026-08-06 GitHub Actions outage with zero check runs. This directly undercuts 3.1–3.3, which just made CI meaningful (integration tests, vitest, and the `@PreAuthorize` e2e suite all now gate nothing). Add a `required_status_checks` rule naming the `Java CI with Maven` jobs (`build`, `code-quality`); consider `strict` (require branch up to date) separately, as it forces a rebase per merge. Note the legacy `branches/main/protection` API returns 404 for this repo — protection lives in rulesets, so check `repos/.../rules/branches/main`, not the classic endpoint. **Do not enable while Actions is degraded** — required checks that never run block every merge. | MEDIUM | P3 | XS | ⏳ |
