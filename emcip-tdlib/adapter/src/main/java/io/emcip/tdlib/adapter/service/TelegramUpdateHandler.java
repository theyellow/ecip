...
public class TelegramUpdateHandler {
    ...
    private void handleNewMessage(TdApi.Update update) {
        if (update instanceof TdApi.UpdateNewMessage newMessage) {
            ...
            log.debug("Received new message from chat {}: {}", message.chatId, 
                message.content instanceof TdApi.MessageText text ? text.text.text : "[non-text]");
            
            Mono.fromRunnable(() -> {
                eventPublisher.publishMessage(message, newMessage)
    ...
    private void handleMessageEdited(TdApi.Update update) {
        if (update instanceof TdApi.UpdateMessageEdited edited) {
            ...
            eventPublisher.publishUpdate(update)
                .subscribeOn(Schedulers.boundedElastic())
                .subscribe(
                    null,
                    error -> log.error("Error publishing edited event: {}", error.getMessage())
                );
        }
    }
    ...
    private void handleMessageDeleted(TdApi.Update update) {
        if (update instanceof TdApi.UpdateMessageDeleted deleted) {
            log.debug("Message {} deleted in chat {}", deleted.messageId, deleted.chatId);
            
            eventPublisher.publishUpdate(update)
                .subscribeOn(Schedulers.boundedElastic())
                .subscribe(
                    null,
                    error -> log.error("Error publishing deleted event: {}", error.getMessage())
                );
        }
    }
    ...
    private void handleChatTitle(TdApi.Update update) {
        if (update instanceof TdApi.UpdateChatTitle title) {
            ...
            eventPublisher.publishUpdate(update)
                ...
                .subscribe(
                    null,
                    error -> log.error("Error publishing chat title event: {}", error.getMessage())
                );
        }
    }
    ...
    private void handleUserUpdate(TdApi.Update update) {
        if (update instanceof TdApi.UpdateUser userUpdate) {
            ...
            eventPublisher.publishUpdate(update)
                ...
                .subscribe(
                    null,
                    error -> log.error("Error publishing user update: {}", error.getMessage())
                );
        }
    }
}
